...
class AtmosM3(Task):
    ...
    def ctrl_transform_with_integral(
        ...
        thrust = (self.B_inv @ wrench_body).clip(-0.5, 0.5)
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        return self._circle_cost(state) + 0.1*jnp.sum(jnp.square(control[:3])) + jnp.sum(self.R_arm * jnp.square(control[3:9])) + jnp.sum(jnp.square(state.qpos[3:9]))
    #def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
    ...
    def _circle_cost(self, state: mjx.Data, radius: float=5.0, period: float = 30.0) -> jax.Array:
        B = 2*jnp.pi/period
